    ...
    <nav className="sticky top-0 z-50 bg-card/95 backdrop-blur-md shadow-lg font-display border-b-4 border-moodoo-yellow">
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={openCart}
            className="relative p-2 rounded-full hover:bg-moodoo-yellow/20 transition-colors"
            aria-label="Giỏ hàng"
          >
            <ShoppingCart className="w-5 h-5 text-moodoo-orange" />
            {totalItems > 0 && (
              <span className="absolute -top-1 -right-1 bg-moodoo-rose text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold animate-bounce">
                {totalItems}
              </span>
            )}
          </button>
          ...
          {isLoggedIn ? (
            ...
            >
              ...
              <span className="text-xs opacity-80">(Thoát)</span>
            </button>
          ) : (
            <button
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-xl hover:bg-moodoo-yellow/20"
            aria-label="Menu"
          >
          ...
          <div className="px-4 pb-4">
            {isLoggedIn ? (
              <button
                ...
                className="w-full bg-moodoo-green text-white px-4 py-3 rounded-xl font-bold text-sm"
              >
                🦔 {user} (Đăng xuất)
              </button>
            ) : (
              <button
                ...
                className="w-full bg-moodoo-rose text-white px-4 py-3 rounded-xl font-bold text-sm"
              >
                ✨ Đăng nhập / Đăng ký
              </button>
            )}
